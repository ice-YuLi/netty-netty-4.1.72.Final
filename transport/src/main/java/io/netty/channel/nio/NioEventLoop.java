...
public final class NioEventLoop extends SingleThreadEventLoop {
    ...
    protected void run() {
        ...
        for (;;) {
            try {
                int strategy;
                try {
                    // 轮询多路复用器，处理i/o事件的，EventLoop线程启动后，在无线循环中 select() 完成的 SelectionKeys，并封装成任务，被分别处理
    ...
    private void processSelectedKeys() {
        //  如果selectedKeys不为空就调用优化过的SelectionKeys的处理方法，不然就处理普通的SelectionKeys集
        //  合，普通的SelectionKey容器为Set,优化过的容器用数组存储
    ...
    private void processSelectedKey(SelectionKey k, AbstractNioChannel ch) {
        ...
        try {
            ...
            // to a spin loop
            // 如果是 OP_READ 或者 OP_ACCEPT
            if ((readyOps & (SelectionKey.OP_READ | SelectionKey.OP_ACCEPT)) != 0 || readyOps == 0) {
                // OP_ACCEPT : AbstractNioMessageChannel.java
                // OP_READ : AbstractNioByteChannel.java
                unsafe.read();
            }
